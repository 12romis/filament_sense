...
class ScaleManager {
 public:
  void begin(const Hx711PinConfig& config, float raw_units_per_gram, long raw_offset_units);
  bool readRawAverage(long& out_raw_sum);
  bool readRaw(long& out_raw);
 ...
 private:
  static constexpr uint8_t kAverageSamples = 20;
